...
services:
  ...
  opalserver:
    ...
    environment:
      ...
      - OPAL_DATA_CONFIG_SOURCES={"config":{"entries":[{"url":"http://opalserver:7002/policy-data","topics":["policy_data"],"dst_path":"/static"}]}}
  ...
  opalclient:
    ...
    environment:
      - OPAL_SERVER_URL=http://opalserver:7002
      - OPAL_LOG_FORMAT_INCLUDE_PID=true
    ...
    depends_on:
      - opalserver
    command: sh -c "./wait-for.sh opalserver:7002 --timeout=20 -- ./start.sh"
```
