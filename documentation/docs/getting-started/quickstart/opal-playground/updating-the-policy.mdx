...
<!-- Due to the fact the example `docker-compose.yml` makes OPAL track [this repository](https://github.com/gvangest/opal-example-policy-repo)
...
track [this repository](https://github.com/gvangest/opal-example-policy-repo).
...
opalserver:
  ...
  environment:
    # the broadcast backbone uri used by opal server workers (see comments above for: broadcastchannel)
    - OPAL_BROADCAST_URI=postgres://postgres:postgres@broadcastchannel:5432/postgres
    ...
    - OPAL_POLICY_REPO_URL=https://github.com/gvangest/opal-example-policy-repo
