...
The default policy in the [example repo](https://github.com/gvangest/opal-example-policy-repo) is a simple [RBAC](https://en.wikipedia.org/wiki/Role-based_access_control) policy, we can issue this request to get the user's role assignment and metadata:
...
The default policy in the [example repo](https://github.com/gvangest/opal-example-policy-repo) is a simple RBAC policy with a twist.
...
Bob is granted access because the initial `data.json` location is `US` ([link](https://github.com/gvangest/opal-example-policy-repo/blob/master/data.json#L18)):
