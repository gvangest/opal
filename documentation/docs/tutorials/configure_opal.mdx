...
- via environnement variables (prefixed with 'OPAL\_')
...
- Look at the configuration code itself:
  ...
  - [Server config](https://github.com/permitio/opal/blob/master/packages/opal-server/opalserver/config.py) : values available only for the server
  - [Client config](https://github.com/permitio/opal/blob/master/packages/opal-client/opalclient/config.py) : values available only for the client
